...
6. 作業ブランチを作成する（ブランチ名: `feat/add-{IDを小文字にしたもの}` 例: `feat/add-a012`）
7. `data/articles/{ID}.md` のみをコミットする（`articles.json` はコミットしない）
8. ブランチをプッシュし、プルリクエストを作成する
   - PRタイトル: `feat: {記事タイトル}の記事を追加`
   - PR本文: 記事のID・タイトル・カテゴリ・概要を記載する
